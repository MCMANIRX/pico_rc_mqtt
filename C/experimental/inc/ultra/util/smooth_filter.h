...
void init_smooth(smooth_filter *inst, int _samples) {
    inst->sum = 0;
    inst->last = 0;
    inst->roc = 0;
...
static int pre_filter_data(smooth_filter *inst, float point, enum pre_filter pf)
{
  if (pf == RANGE)
  {
...
float filter_average(smooth_filter *inst, float point, enum pre_filter pf) {
    inst->sum = 0;
...
float filter_average_kalman(smooth_filter *inst, float point, enum pre_filter pf) {
    inst->sum = 0;
...
float filter_median(smooth_filter *inst, float point, enum pre_filter pf) {
...
float filter_median_kalman(smooth_filter *inst, float point,enum pre_filter pf) {
  // No need to reset sum here, median doesn't use it
...
float filter_kalman(smooth_filter *inst, float point,enum pre_filter pf){
