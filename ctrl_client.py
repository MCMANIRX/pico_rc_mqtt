...
#        client.publish(rc.ENQUIRE_TOPIC,rc.ENQ) # ENQ
    ...
    if controller_sample:
        _x = controller_sample[0]
        a_now = controller_sample[10]&2
        b_now = controller_sample[10]&4
        x_now = controller_sample[10]&1
        ...
        if(ctrl_mode == 0):
            _y = controller_sample[2]
            y = rc.arduino_map(_y,min_y,max_y,-127,127,deadzone_y)
        
        elif(ctrl_mode == 1):
            _y = controller_sample[9] # reversed since -1 = forward
            y = -rc.arduino_map(int(_y),255,0,-127,127,128)
           # print(y)
