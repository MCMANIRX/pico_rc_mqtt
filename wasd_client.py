...
#        client.publish(rc.ENQUIRE_TOPIC,rc.ENQ) # ENQ
    ...
    if keys_down:
        ...
        x = turn #rc.arduino_map(int(_x),0,255,-127,127,deadzone_x)
        ...
        if (x or  _y) and a_presses == 0 and gpd0.timeout() and not script_active:
         #s   print("legos")
            sendctrl(client,x,y,"joy")
            gpd0.delay_ms(1000/60)
    else:
        sendctrl(client,0,0,"joy")
        gpd0.delay_ms(1000/60)
